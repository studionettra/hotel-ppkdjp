...
export default function Edit({ reservation, guests, roomTypes }) {
    ...
    return (
        <AppLayout title="Edit Reservasi" breadcrumbs={[
            ...
            <div className="row">
                <div className="col-md-8">
                    <div className="card">
                        <div className="card-header"><h4 className="card-title">Edit: {reservation.reservation_code}</h4></div>
                        <div className="card-body">
                            <form onSubmit={submit}>
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <select className={`form-select${errors.room_type_id ? ' is-invalid' : ''}`}
                                            ...
                                            {roomTypes.map(rt => <option key={rt.id} value={rt.id}>{rt.name} (maks. {rt.max_capacity})</option>)}
                                        </select>
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        {data.check_in_date && <small className="text-muted">{formatTanggal(data.check_in_date)}</small>}
                                    </div>
                                    <div className="col-md-6">
                                        ...
                                        {data.check_out_date && <small className="text-muted">{formatTanggal(data.check_out_date)}</small>}
                                    </div>
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <select className="form-select" value={data.channel} onChange={e => setData('channel', e.target.value)}>
                                            {['walk_in','phone','website','ota','email'].map(c => <option key={c} value={c}>{c}</option>)}
                                        </select>
                                    </div>
                                ...
                                <div className="d-flex gap-2 mt-4">
                                    ...
                                    <Link href={route('reservations.show', reservation.id)} className="btn btn-secondary">Batal</Link>
                                </div>
                            </form>
                        </div>
                    </div>
